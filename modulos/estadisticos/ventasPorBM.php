        ...
        <div id="one"> <form action="" method="GET" id="horizontalForm">
            ...
            <fieldset>
				<legend>Filtros</legend>
							 ...
							 <input name="opc" type="hidden" id="opc" size="40" class="required" value="ventasPorBM" />
				 ...
				 if($tipoProducto)
				 {
				  ...
				  if($marca)
				 {
				    $lineaTablaOn = ",TABLA.Linea               [Linea]";
				    $lineaOn = ",PV.Linea";
					$lineaGroupON = ",TABLA.Linea";
					$Wmarca = " AND Marca = '".$marca."'";
				 }
...
$sql="
SELECT
       TABLA.BrandManager        [BrandManager]
	  ".$lineaTablaOn."
...
overflow-y:hidden;">				
            <table   width="2500px;" id="tablelike"  class="lista">
              <thead>
				...
				<tr>
						<th>Informe de Ventas por Brand Manager <?php echo $periodo;  ?> 
                        <?php
						if($marca)
						echo "- Marca: ".$marca." "; 
						if($tipoProducto)
                   ...
                   <tr>
						...
						<th>Total Venta UNDS</th> 
						<th>Total Venta CLP</th> 
						<th>Total Venta USD</th> 
						<th>Total Venta CIF</th> 
		...
		<?php
					...
					while($resultado = odbc_fetch_array($rs))
					{
						...
						echo'
						<tr>
						<td >'.utf8_encode((string)$resultado["BrandManager"]).'</td>';
						
						if($marca)
						echo '<td >'.utf8_encode((string)$resultado["Linea"]).'</td>';
						...
						<td ><strong>'.number_format($resultado["Total_USD"]-$resultado["CtoVtaCIF"], 4, ',', '.').'</strong></td>
						...
						$tMargenBruto = $tMargenBruto + ($resultado["Total_USD"]-$resultado["CtoVtaCIF"]);
		...
		<?php
			echo'<tr style="border-top:2px solid #575757;">
						...
						<td ><strong>'.number_format($tMargenBruto, 4, ',', '.').'</strong></td>
