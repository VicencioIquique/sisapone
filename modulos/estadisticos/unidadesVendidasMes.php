        ...
        <div id="one"> <form action="" method="GET" id="horizontalForm">
            ...
            <fieldset>
				<legend>Filtros</legend>
							 ...
							 <input name="opc" type="hidden" id="opc" size="40" class="required" value="unidadesVendidasMes" />
...
SELECT
      ...
      +SUM(TABLA.Octubre)+SUM(TABLA.Noviembre)+SUM(TABLA.Diciembre) [Total_UNDS]
...
(
		SELECT  
		  ...
		  FROM [SBO_Imp_Eximben_SAC].[dbo].[VIC_VW_Vtas_Unds_Per_CIf]
		  pivot (
				 SUM([Quantity])
				 for [Periodo] in 
				  ([".$periodo."-01] 
...
overflow-y:hidden;">				
		...
		<?php
					...
					while($resultado = odbc_fetch_array($rs))
					{
						...
						<td ><strong>'.number_format($resultado["3er_Trim"], 0, '', '.').'</strong></td>
						<td style=" border-right:1px solid #575757;"><strong>'.number_format($resultado["4to_Trim"], 0, '', '.').'</strong></td>
		...
		<?php
			echo'<tr style="border-top:2px solid #575757;">
						...
						<td ><strong>'.number_format($ttrim3, 0, '', '.').'</strong></td>
						<td style=" border-right:1px solid #575757;"><strong>'.number_format($ttrim4, 0, '', '.').'</strong></td>
