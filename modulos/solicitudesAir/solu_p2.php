...
ini_set('max_execution_time', 300); //300 seconds = 5 minutes
 ...
 $sql="SELECT   estado
 	   FROM     [RP_REGGEN].dbo.sisap_solicitudes WHERE solicitud_id LIKE '".$idsol."'";
...
?>
 ...
 <script type="text/javascript">
  $(document).ready(function() {
	...
	fn_anade_listaAir();	//eliminar de lista
...
$(document).ready(function() {
	fn_eliminar_articulolistaAir();	//eliminar de lista
...
FROM         (SELECT     Alu, Cantidad
                       FROM          [RP_REGGEN].dbo.VerStockTiendas
                       WHERE      (Bodega = '00".$modulo."') ) AS TABLA FULL OUTER JOIN
                      dbo.oITM_From_SBO ON dbo.oITM_From_SBO.ItemCode = TABLA.Alu COLLATE SQL_Latin1_General_CP850_CI_AS LEFT OUTER JOIN
                      dbo.View_OMAR ON dbo.View_OMAR.Code = dbo.oITM_From_SBO.U_VK_Marca
WHERE     (dbo.View_OMAR.Name = '".$bmarca."')  AND (dbo.oITM_From_SBO.ItmsGrpCod <> 103)  AND 
                      (dbo.oITM_From_SBO.ItmsGrpCod <> 100) AND (dbo.oITM_From_SBO.ItmsGrpCod <> 106) AND (dbo.oITM_From_SBO.ItmsGrpCod <> 107) AND 
                      (dbo.oITM_From_SBO.ItmsGrpCod <> 104) 
  ".$Wcosto."
              ...
              <thead>
                ...
                <tbody id="tablebody">
                   <?php
				 	...
				 	$sql="SELECT     [RP_REGGEN].dbo.sisap_solicitudes.solicitud_id, [RP_REGGEN].dbo.sisap_soldetalle.solicitud_id AS detid, [RP_REGGEN].dbo.sisap_soldetalle.codigo, [RP_REGGEN].dbo.sisap_soldetalle.descripcion, 
                      [RP_REGGEN].dbo.sisap_soldetalle.marca, [RP_REGGEN].dbo.sisap_soldetalle.stock_modulo, [RP_REGGEN].dbo.sisap_soldetalle.cant_solicitada, [RP_REGGEN].dbo.sisap_soldetalle.cant_aceptada, 
                      [RP_REGGEN].dbo.sisap_soldetalle.detalle_id
FROM         [RP_REGGEN].dbo.sisap_solicitudes LEFT OUTER JOIN
                      [RP_REGGEN].dbo.sisap_soldetalle ON [RP_REGGEN].dbo.sisap_solicitudes.solicitud_id = [RP_REGGEN].dbo.sisap_soldetalle.solicitud_id
WHERE     ([RP_REGGEN].dbo.sisap_solicitudes.solicitud_id = ".$idsol.")";
            ...
            <div id="" style="float:right; font-size:30px; margin-right:20px; ">
				...
				if($estado ==0)
				{
				echo ' <input type="button" value="Enviar"  class="submit" style=" float:left; margin-top:0px;" onclick="javascript: fn_finalizar_guiaAir();" />';	
				}
				if($estado ==1)
				{
				echo ' <input type="button" value="Validar"  class="submit" style=" float:left; margin-top:0px;" onclick="javascript: fn_finalizar_guiaAir();" />';	
				}
				?>
    			</div><!-- fin   -->
