	 ...
	 if($registro>0){
			...
			// echo "estoy aqui";
				...
				if($resultado["usuario_pass"]==$pass)//crypt($pass,"semilla") )
				{
					 ...
					 $_SESSION["linkPersonal"] = $resultado["linkDirectorio"];
				  	 $_SESSION["bodega"] = $resultado["bodega"];
					 $_SESSION["workstation"] = $resultado["workstation"];
                        ...
                        generaLogs($resultado["usuario_nombre"],$accion,$origen);
							echo '<script languaje="javascript"> window.location="index.php?opc=#"; </script>';
				}

				else { 
					$accion="PASSWORD INCORRECTO - INICIO DE SESION";
   					 $origen=$_SERVER['REMOTE_ADDR'] .' - '.gethostbyaddr($_SERVER['REMOTE_ADDR']).' - '.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
    				generaLogs($resultado["usuario_nombre"],$accion,$origen);
