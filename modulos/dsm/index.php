      ...
      <div class="items">
        ...
        <div id="one">
         <form action="" method="post" id="horizontalForm">
            ...
            <fieldset>
				<legend>Listado de Entradas de Mercadería <?php echo getmodulo((int)$modulo);?> </legend>
                    <label for="pais1">
                ...
                <tbody>
                 <?php
				 	  ...
				 	  $sql="SELECT [NroDEM] ,[TOrigen],[CodModulo] AS modulo ,CONVERT(varchar, [FechaDoc], 103) AS Fecha,count([NroDEM]) AS TotalItem,Estado 
							  FROM [RP_VICENCIO].[dbo].[RP_DEM] WHERE [CodModulo] = ".$modulo." AND CodModulo <> 'TP8' AND CodModulo <> 'TP2' 
					...
					$cantotal =0;
							...
							//  echo $resultado["estado"];
								...
								if($resultado["Estado"]>0)
								{
									...
									{
							   echo '<tr>
									<td style="background-color:#6C6B6B;color:#fff;font-weight:bold; font-size:15px; width:23px; text-align:center;" ><a style="color:#fff;"   href="index.php?opc=validarDSM&nDsm='.$resultado["NroDEM"].'">'.utf8_encode($resultado["NroDEM"]).'</a></td>';
									}
									echo '
								    <td ><strong>'.$resultado["Fecha"].'</strong></td>
									<td >'.$resultado["TOrigen"].'</td>
									<td >'.getmodulo((int)$resultado["modulo"]).'</td>
								...
								{
							   echo '<tr>
									<td style="background-color:#5484C7;color:#fff;font-weight:bold; font-size:15px; width:23px; text-align:center;" ><a  style="color:#fff;" href="index.php?opc=validarDSM&nDsm='.$resultado["NroDEM"].'">'.utf8_encode($resultado["NroDEM"]).'</a></td>
									<td ><strong>'.$resultado["Fecha"].'</strong></td>
									<td >'.$resultado["TOrigen"].'</td>
									<td >'.getmodulo((int)$resultado["modulo"]).'</td>
									<td >'.$resultado["TotalItem"].'</td>' ;
