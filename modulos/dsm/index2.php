...
WHERE [NroDEM] = '".$nDsm."' AND [CodModulo] = ". $_SESSION["usuario_modulo"]."
  ...
  $sql="SELECT [NroDEM]  ,[TOrigen],BO.bodega ,CONVERT(varchar, [FechaDoc], 103) AS Fecha,count([NroDEM]) AS TotalItem,Estado  
FROM [RP_VICENCIO].[dbo].[RP_DEM] 
LEFT JOIN [dbo].[Bodega] BO ON CodModulo = BO.Retail
WHERE [NroDEM] = '".$nDsm."' AND [CodModulo] = ". $_SESSION["usuario_modulo"]."
GROUP BY NroDEM,[FechaDoc],BO.bodega,[TOrigen],Estado
ORDER BY FechaDoc DESC";
         ...
         <form action="" method="GET" id="horizontalForm">
            <fieldset >
				<legend>Ingrese Nro. Documento Salida a Módulos</legend>
					<input name="opc" type="hidden" id="opc" size="40" class="required" value="validarDsm" />
				  ...
				  <?php
							 	if($_SESSION["usuario_modulo"] == -1)
								{
									...
									<option value="008">Modulo 2077 EXB</option>
									</select>
				            </label>';
								}
