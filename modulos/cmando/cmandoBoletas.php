...
<div class="idTabs">
  ...
  <div id="caja2" style="padding-top:20px;">
		...
		<div style="float:left;width:30%; height: 250px;">
			...
			<!--<div>
				<center>
					<label style="margin-top:px;">Valor CLP efectivo<br><label id="cajaExiSinPago"></label></label>
				</center>
			</div>-->
		</div>
		<div  style="float:left;width:30%; height: 250px;">
			...
			<!--<div>
				<center>
					<label style="margin-top:px;">Valor CLP efectivo<br><label id="cajaExiConPago"></label></label>
				</center>
			</div>-->
		</div>
		<div  style="float:left; width:33.3%; height: 250px; margin-top:-50px;">
			...
			<!--<div>
				<center>
					<label style="margin-top:px;">Valor CLP efectivo<br><label id="cajaExiSinCentralizarAir"></label></label>
				</center>
			</div>-->
		</div>
		<div style="float:left;width:30%; height: 250px; margin-top:-50px; ">
			...
			<!--<div>
				<center>
					<label style="margin-top:px;">Valor CLP efectivo<br><label id="cajaExiSinPagoAir"></label></label>
				</center>
			</div>-->
		</div>
		<div  style="float:left;width:30%; height: 250px; margin-top:-50px;">
			...
			<!--<div>
				<center>
					<label style="margin-top:px;">Valor CLP efectivo<br><label id="cajaExiConPagoAir"></label></label>
				</center>
			</div>-->
		</div>
   ...
   <div style="clear:both; padding-left:20px;"><label style="font-size:18px; color:#BB9999;">VALOR EN PESOS(CLP) DE CAJAS:</label></div><br>
   <div style="padding-left:35px; float:left; width:33%;">
	<h4>Caja Locales Eximben Mall Zofri</h4>
		<?php
			$sql = "SELECT AcctName, REPLACE(CurrTotal, '.',',') AS SaldoActualCLP, CurrTotal, ActCurr, Segment_0, Segment_1, FormatCode, FatherNum
					FROM SBO_Imp_Eximben_SAC.dbo.OACT
					WHERE FatherNum='1101' AND FormatCode='110109000'";
					$rs = odbc_exec( $conn, $sql );
					if(!$rs ){
						exit( "Error en la consulta SQL" );
					}
					$resultado = odbc_fetch_array($rs);
					echo "<label style='font-size:20px;'>CLP $".number_format($resultado['SaldoActualCLP'],0,'.',',')."</label>";
		?>
   </div>
   <div style="padding-left:20px; float:left; width:33%;">
	<h4>Caja Locales Servimex Mall Zofri</h4>
		<?php
			$sql = "SELECT AcctName, REPLACE(CurrTotal, '.',',') AS SaldoActualCLP, CurrTotal, ActCurr, Segment_0, Segment_1, FormatCode, FatherNum
   ...
   <div style="padding-left:20px; float:left;">
	<h4>Caja Locales Eximben Aeropuerto</h4>
		<?php
			$sql = "SELECT AcctName, REPLACE(CurrTotal, '.',',') AS SaldoActualCLP,CurrTotal, ActCurr, Segment_0, Segment_1, FormatCode, FatherNum
					FROM SBO_Eximben_RegGen.dbo.OACT
					WHERE FatherNum='1101'";
					$rs = odbc_exec( $conn, $sql );
					if(!$rs ){
						exit( "Error en la consulta SQL" );
					}
