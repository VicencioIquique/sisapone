...
$sql3=" SELECT     dbo.sisap_soldetalle.detalle_id, dbo.sisap_soldetalle.descripcion, dbo.sisap_soldetalle.marca, dbo.sisap_soldetalle.stock_modulo, 
                      dbo.sisap_soldetalle.cant_solicitada, dbo.sisap_soldetalle.cant_aceptada, dbo.sisap_soldetalle.solicitud_id, dbo.sisap_soldetalle.codigo 
                     
FROM         dbo.sisap_soldetalle 
WHERE     (dbo.sisap_soldetalle.solicitud_id = ".$idsol.") ";
		...
		exit( "Error en la consulta SQL" );
				...
				$pdf->Cell(8,6,'Cant/P',1,0,'C',1);
				$pdf->SetX(153);
				$pdf->Cell(8,6,'13-1',1,0,'C',1);
				$pdf->SetX(161);
				$pdf->Cell(8,6,'13-2',1,0,'C',1);
				$pdf->SetX(169);
				$pdf->Cell(8,6,'14-6',1,0,'C',1);
				$pdf->SetX(177);
				$pdf->Cell(8,6,'13-6',1,0,'C',1);
				$pdf->SetX(185);
				$pdf->Cell(8,6,'17SZ',1,0,'C',1);
				$pdf->SetX(193);
				$pdf->Cell(8,6,'1623',1,0,'C',1);
				$pdf->SetFont('Arial','B',8);
				...
				while($row = odbc_fetch_array($rs3)){ 
					...
					$pdf->SetX(9);
					$pdf->SetFont('Arial','',7);
					$pdf->Cell(22,6,$row["codigo"],1,0,'C',0);
					$pdf->SetY($Y_Table_Position);
					$pdf->SetX(31);
					...
					$pdf->SetY($Y_Table_Position);
					$pdf->SetX(145);
					$pdf->Cell(8,6,$row["cant_aceptada"],1,0,'C',0);
					$pdf->SetX(153);
					$pdf->Cell(8,6,$bod[0],1,0,'C',1);
					$pdf->SetX(161);
					$pdf->Cell(8,6,$bod[1],1,0,'C',1);
					$pdf->SetX(169);
					$pdf->Cell(8,6,$bod[2],1,0,'C',1);
					$pdf->SetX(177);
					$pdf->Cell(8,6,$bod[3],1,0,'C',1);
					$pdf->SetX(185);
					$pdf->Cell(8,6,$bod[4],1,0,'C',1);
					$pdf->SetX(193);
					$pdf->Cell(8,6,$bod[5],1,0,'C',1);
